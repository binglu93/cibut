...
function loadVpsList() {
  ...
  if (!fs.existsSync(p)) throw new Error('Upss Server tidak ditemukan.');
...
// ===== inline keyboard VPS (versi lengkap + batal) =====
    ...
    conn.on('ready', () => {
      conn.exec(shellCmd, (err, stream) => {
        if (err) {
          ...
          finished = true;
          send(bot, msg.chat.id, '❌ Gagal menjalankan perintah di Server.').catch(()=>{});
          conn.end();
...
function createAddSshPlugin({ name, aliases=[], title, commandTpl, expMode='days', hargaPerHari=0 }) {
  ...
  async function start(bot, msg){
    ...
    setTimeout(() => {
      ...
      if (S && S.step === 1) {
        ...
        send(bot, msg.chat.id, '⏳ Sesi SSH sudah kadaluarsa.').catch(()=>{});
      }
    }, 60_000);
  }
  ...
  async function cont(bot, msg){
    ...
    if (/^([./])?batal$/i.test(t)) {
      delete global.__addssh_sessions[key];
      await send(bot, msg.chat.id, '✅ Sesi SSH dibatalkan.');
      return true;
    }
